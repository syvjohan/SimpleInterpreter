...
namespace Partitioning {
	...
	char* Parser::RegularExpression( char *expression ) {
		...
		if( op0.pos <= 0 ) {
			Global::Alias_s alias;
			if( expression[ 0 ] == '&' ) {
				const int address = heap.GetAddress( expression + 1 );
				sprintf( alias.value, "%d", address );
			} else {
			...
			if( op1.pos == -1 ) {
				...
				if( eq ) {
					...
					if( !cmpEq ) {
						if( !Global::HelpClass::StrCmp( op1.op, "=" ) ) {
							lenRhs = op0.pos - op1.pos - op0.len;
							op0 = op1;
							op1.pos = -1;
							offset = 0;
							isRhsEval = false;
				...
				if( !Global::HelpClass::IsNegativeNumber( newExpression ) ) {
					memcpy( Buffer2, expression + op0.pos + op0.len, lenRhs );
					Buffer2[ lenRhs ] = '\0';
					lenRhs = strlen( Buffer2 );

					if( op1.pos == -1 ) {
						memcpy( buffer, expression, op0.pos );
						buffer[ op0.pos ] = '\0';
					} else {
						memcpy( buffer, expression + op1.pos + op1.len, lenNewExp - lenRhs - offset );
				...
				if( isRhsEval ) {
					...
					buffer[ strlen(buffer) ] = '\0'; //len - lenRes + 1
				}
	...
	char* Parser::CalculateResult( const char *lhs, const char *rhs, const char *op ) {
		...
		if( Global::HelpClass::StrCmp( op, "=" ) && aliasLhs.name[ 0 ] != '\0' ) {
			...
			if( lhs[ 0 ] == '&' || lhs[ 0 ] == '#' ) {
				int len = strlen( aliasRhs.type );
				memcpy( aliasLhs.type, aliasRhs.type, len );
				aliasLhs.type[ len ] = '\0';
		...
		if( ( lhs[ 0 ] == '#' && Global::HelpClass::StrCmp( op, "+" ) ) || ( lhs[ 0 ] == '#' && Global::HelpClass::StrCmp( op, "-" ) ) ) {
			...
			if( Global::HelpClass::CheckForDigits( aliasLhs.value ) && Global::HelpClass::StrCmp( aliasRhs.type, "int" ) ) {
				//if user wants to traverse memory addresses.
		...
		if( ( lhs[ 0 ] == '&' && Global::HelpClass::StrCmp( op, "+" ) ) || ( lhs[ 0 ] == '&' && Global::HelpClass::StrCmp( op, "-" ) ) ) {
			...
			memcpy( aliasLhs.value, lhs + 1, len );
			aliasLhs.value[ len ] = '\0';
			if( Global::HelpClass::CheckForDigits( aliasRhs.value ) && Global::HelpClass::StrCmp( aliasRhs.type, "int" ) ) {
				//if user wants to traverse memory addresses.
				const int address = heap.GetAddress( aliasLhs.name );
				if( Global::HelpClass::StrCmp( op, "+" ) ) {
					sprintf( tmpStr, "%d", address + atoi( aliasRhs.value ) );
				} else {
					sprintf( tmpStr, "%d", address - atoi( aliasRhs.value ) );
				}
			}
		...
		//Both are text strings.
			...
			if( Global::HelpClass::StrCmp( op, "+" ) ) {
				...
				//Check if lhs is address number or alias name7
				int address;
				if ( Global::HelpClass::CheckForDigits( lhs ) == 1 ) { //== 1 //aliasLhs.name
					address = atoi( lhs + 1 );
					//address = atoi( aliasLhs.name );
					aliasLhs.name[ 0 ] = '\0';
					heap.InsertAliasAt( address, aliasLhs );
				} else {
					address = heap.GetAddress( aliasLhs.name );
					heap.InsertAliasAt( address, aliasLhs );
				}
		...
		} else if ( Global::HelpClass::StrCmp( aliasLhs.type, "string" ) && Global::HelpClass::StrCmp( aliasRhs.type, "int" ) ) {
		...
		} else if ( aliasLhs.len == 0 && Global::HelpClass::StrCmp(aliasRhs.value, "\\0")) {
			if ( Global::HelpClass::StrCmp( op, "==" )  ) {
				memcpy( tmpStr, "true", 4 );
				tmpStr[4] = '\0';
	...
	Global::Alias_s Parser::ParseKeywords( char *exp ) {
		...
		if( a.len > 0 ) {
			...
			tmpStr[ len ] = '\0';
			memcpy( alias.name, tmpStr, strlen( tmpStr ) );
			alias.name[ strlen( tmpStr ) ] = '\0';


			const int a = atoi( tmpStr );
			alias = heap.GetAlias( a );
			if( alias.len == 0 && alias.type[ 0 ] == '\0' && alias.value[ 0 ] == '\0' ) {
